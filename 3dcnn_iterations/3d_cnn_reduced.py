...
# Halved filter counts reduce overfitting on 267 training examples; checkpoints on best val accuracy.
...
# Spatial progression (filter counts halved vs strided variant):
#   Input        (1,  96, 96, 96)
...
def main():
    ...
    results_file = results_dir / f"reduced_{timestamp}.csv"
    ...
    for epoch in range(1, epochs + 1):
        ...
        epoch_rows.append({
            ...
            "checkpoint": "best" if val_acc == best_val_acc else "",
        })

    print(f"\nBest val acc {best_val_acc:.4f} at epoch {best_epoch} — restoring for test.")
