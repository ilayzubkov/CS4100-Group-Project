...
# Best-model config (WRS + sqrt loss weights + macro checkpointing) with flip/noise/intensity augmentation added.
...
class OASISDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            # With WRS drawing each C2 volume ~3.56x per epoch, noise ensures each draw
            # is a distinct sample rather than an identical repeat.
            if torch.rand(1).item() < 0.5:
...
def categorical_cross_entropy(logits, targets, class_weights):
    """
    Manual softmax cross-entropy with sqrt inverse-frequency per-class scaling.
...
def main():
    ...
    results_file = results_dir / f"extended_aug_{timestamp}.csv"
    ...
    # Sqrt loss weights on top of oversampling: sampler handles frequency,
    # weights provide an additional gradient push toward rare classes.
    class_weights = torch.tensor(
        [(total / (3 * c)) ** 0.5 for c in class_counts], dtype=torch.float32
    ).to(DEVICE)
