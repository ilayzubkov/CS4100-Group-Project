...
# Adam (lr=1e-4), gradient clipping (norm=1.0), sqrt inverse-frequency weights, flip augmentation.
...
# Spatial progression:
#   Input        (1,  96, 96, 96)
#   After block1 (8,  48, 48, 48)   <- stride=2
#   After block2 (16, 24, 24, 24)   <- stride=2
#   After block3 (32, 12, 12, 12)   <- stride=2
#   After block4 (64,  6,  6,  6)   <- stride=2
...
def categorical_cross_entropy(logits, targets, class_weights):
    ...
    Uses sqrt inverse-frequency weights: weight_c = sqrt(total / (3 * count_c)).
    Preserves the relative ordering of raw inverse-frequency weights while reducing
    the gradient magnitude of rare-class examples that was causing training instability.
    """
...
def train_one_epoch(model, loader, optimizer, class_weights, max_grad_norm):
    """
    One pass over the training set with gradient clipping.

    Clips the global gradient norm to max_grad_norm before each optimizer step,
    preventing large updates from the class-weighted loss from destabilising training.
...
def main():
    ...
    learning_rate = 1e-4        # reduced from 1e-3; smaller steps + clipping avoids overshooting
    batch_size    = 4
    epochs        = 50
    dropout_p     = 0.2
    max_grad_norm = 1.0
    ...
    results_file = results_dir / f"gradclip_{timestamp}.csv"
    ...
    # Sqrt inverse-frequency weights: sqrt(total / (3 * count_c))
    ...
    print(f"lr={learning_rate}  grad_clip={max_grad_norm}")
