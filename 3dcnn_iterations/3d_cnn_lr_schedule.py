...
class OASISDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            # Left-right flip: Alzheimer's pathology is bilateral, so the label is invariant.
            if torch.rand(1).item() < 0.5:
                volume = torch.flip(volume, dims=[3])

            # Gaussian noise: simulates MRI thermal noise. sigma=0.05 is ~5% of one
            # standard deviation on z-scored volumes — perturbs without destroying structure.
            if torch.rand(1).item() < 0.5:
                volume = volume + 0.05 * torch.randn_like(volume)

            # Intensity scaling: simulates scanner gain variation across sessions.
            # Scale drawn from Uniform(0.9, 1.1) each time, so repeated C2 draws differ.
            if torch.rand(1).item() < 0.5:
...
def macro_accuracy(per_class_dict):
    ...
    and the LR scheduler signal because val has only 6 C2 examples (11%) while test
    has 13 (25%) — overall accuracy underweights C2 as a convergence signal.
    """
...
def main():
    ...
    results_file = results_dir / f"lr_schedule_{timestamp}.csv"
    ...
    # Equal loss weights — the sampler already balances class frequency.
    ...
    # Halve LR when val macro accuracy hasn't improved for 15 epochs.
    # mode='max' because we are maximising macro accuracy, not minimising a loss.
    ...
    print(f"LR scheduler: ReduceLROnPlateau  factor=0.5  patience=15  min_lr=1e-6")
    print(f"Augmentation: flip(p=0.5)  noise(sigma=0.05, p=0.5)  intensity_scale(Uniform(0.9,1.1), p=0.5)")
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Step the scheduler with the current val macro accuracy.
        # LR is halved if macro hasn't improved for 15 consecutive epochs.
        ...
        print(f"Epoch {epoch:>3}/{epochs} | "
              ...
              f"{pc_str}  lr {current_lr:.2e}{checkpoint_marker}")
    ...
    fieldnames = [
        ...
        "val_loss", "val_acc", "val_macro", "val_c0", "val_c1", "val_c2", "lr", "checkpoint",
    ...
    with open(results_file, "w", newline="") as f:
        ...
        writer.writerow({
            ...
            "val_c0": "", "val_c1": "", "val_c2": "", "lr": "", "checkpoint": "",
